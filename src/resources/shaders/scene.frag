...
void main()
{
	...
	for(int i = 0; i < MAX_POINT_LIGHTS; i++ ) {
		if(pointLights[i].intensity > 0 ) diffuseSpecularComp += calcPointLight(diffuse, specular, pointLights[i], outPosition, outNormal);
	}
	
	for(int i = 0; i < MAX_SPOT_LIGHTS; i++ ) {
		if(spotLights[i].pl.intensity > 0 ) diffuseSpecularComp += calcSpotLight(diffuse, specular, spotLights[i], outPosition, outNormal);
	}
