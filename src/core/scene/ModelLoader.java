...
public class ModelLoader {
	...
	public static Model loadModel(String modelId, String modelPath, TextureCache textureCache, int flags) {
		...
		if(!file.exists()) throw new RuntimeException("Model path does not exist [" + modelPath + "]");
		
		String modelDir = file.getParent();;
		
		AIScene aiScene = aiImportFile(modelPath, flags);
		if(aiScene == null) throw new RuntimeException("Error loading model [modelPath: " + modelPath + "]");
		...
		for(int i = 0; i < numMeshes; i++) {
			...
			Material material;
			if(materialIdx >= 0 && materialIdx < materialList.size()) material = materialList.get(materialIdx);
			else material = defaultMaterial;
			
			material.getMeshList().add(mesh);
		}
		
		if(!defaultMaterial.getMeshList().isEmpty()) materialList.add(defaultMaterial);
	...
	private static Material processMaterial(AIMaterial aiMaterial, String modelDir, TextureCache textureCache) {
		...
		try(MemoryStack stack = MemoryStack.stackPush()) {
			...
			if(result != aiReturn_SUCCESS) reflectance = shininessFactor[0];
